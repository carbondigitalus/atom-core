...
export const Fragment = (p: { children?: Children }): Children | null => p.children ?? null;

export function jsx<P = Props>(type: ElementType<P>, props?: P & Props, key?: Key): VNode<P> {
  const src = (props ?? {}) as Props;
  const children = (src as { children?: Children }).children;

  const rest = Object.fromEntries(Object.entries(src).filter(([k]) => k !== 'children')) as P & Props;
...
export function jsxs<P = Props>(type: ElementType<P>, props?: P & Props, key?: Key): VNode<P> {
  const src = (props ?? {}) as Props;
  const children = (src as { children?: Children }).children;
